...
function BossModel({ kind, defeated, reducedMotion }) {
  ...
  const surface = useMemo(() => {
    ...
    return new THREE.MeshStandardMaterial({
      ...
      metalness: metallic,
      roughness: rough,
      // Just enough self-illumination that no face reads as a black hole,
      // without tinting the whole model with the eye colour.
  ...
  return (
    <group ref={group}>
      ...
      <pointLight color={glow} intensity={defeated ? 0.4 : 4.5} distance={3.0} position={[0, 0.62, 0.42]} />
    </group>
  );
}
...
function Scene({ kind, defeated, reducedMotion }) {
  return (
    <>
      <fog attach="fog" args={["#241a10", 4.6, 9.0]} />
      ...
      <directionalLight position={[3.0, 1.4, -2.6]} intensity={2.4} color="#8fa8e0" />
      {/* Sky-to-ground fill: warm from above, cool from below. Softer than
          another directional light, and it stops the underside of every
          surface falling to pure black. */}
      <hemisphereLight args={["#ffd9a0", "#2a3550", 1.5]} />
      <ambientLight intensity={0.9} color="#6b5a44" />
      {/* Warm bounce from below, like torchlight off a floor */}
      <pointLight position={[0, -1.4, 1.6]} intensity={3.2} color="#c8783a" distance={6} />
