...
function BossViewer({ kind, defeated, height = 150, reducedMotion }) {
  const canvasRef = useRef(null);
  ...
  useEffect(() => {
    ...
    function draw(now) {
      const time = (now - t0) / 1000;
      const spin = reducedMotion ? 0.5 : time * 0.7;
      // A slow menacing heave, heavier than the character's breathing
      const heave = reducedMotion ? 0 : Math.sin(time * 1.6) * 0.020;
      const lean = reducedMotion ? 0 : Math.sin(time * 0.8) * 0.030;
      const slump = defeatedRef.current ? 0.55 : 0;
      gl.clearColor(0, 0, 0, 0);
      gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
      const model = m4mul(
        m4trans(0, 0.02 + heave - slump * 0.35, -3.05),
        m4mul(m4rotX(0.12 + lean + slump), m4rotY(spin))
      );
      gl.uniformMatrix4fv(uModel, false, model);
      gl.drawArrays(gl.TRIANGLES, 0, verts.length / 11);
      if (!reducedMotion) raf = requestAnimationFrame(draw);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [kind, height, reducedMotion]);

  return <canvas key={kind} ref={canvasRef} className="rlxp-boss-canvas" style={{ height }} />;
}
  ...
  return (
    <div className={`rlxp-boss-panel ${boss.defeated ? "rlxp-boss-defeated" : ""}`}>
      ...
      <div className="rlxp-boss-stage">
        <BossViewer kind={boss.kind} defeated={boss.defeated} reducedMotion={reducedMotion} />
        {lastHit != null && (
